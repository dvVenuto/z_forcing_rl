...
class Z_Forcing_mlp(object):
    def __init__(self, input_dim, embedding_dim, rnn_dim, mlp_dim, z_dim, out_dim, output_type="gaussian",
                 cond_ln=False, num_layers=1, z_force=True, embedding_dropout=0.):
        ...
        self.fwd_mod = tf.keras.layers.Dense(self.rnn_dim)
        ...
        self._aux_mod = tf.keras.Sequential([
            tf.keras.Input(shape=(self.z_dim + self.embedding_dim)),
            tf.keras.layers.Dense(self.mlp_dim, activation='relu'),
            tf.keras.layers.Dense(units=2 * self.rnn_dim),
        ])
        ...
        self._inf_mod = tf.keras.Sequential([
            tf.keras.Input(shape=(self.rnn_dim + self.embedding_dim)),
            tf.keras.layers.Dense(self.mlp_dim, activation='relu'),
    ...
    def fwd_pass(self, x_fwd, bwd_states=None, z_step=None):
        with tf.name_scope('forward-pass'):
            ...
            for step in range(n_steps):
                ...
                if bwd_states is not None:
                    ...
                    if step != 0:
                        x_prev = self.x_fwd[:, step - 1]
                    else:
                        x_prev=tf.zeros([x_step.shape[0], x_step.shape[1]])

                    b_step = bwd_states[:, step]
                    print(b_step.shape)


                    print(tf.concat((x_prev, b_step), axis=1))

                    inf_params = self._inf_mod(tf.concat((x_prev, b_step), axis=1))
                    ...
                    pri_input = tf.concat((z_step, x_prev), axis=1)

                    print(pri_input.shape)

                    pri_params = self._pri_mod(pri_input)
                    ...
                    aux_params = self._aux_mod(tf.concat((x_prev, z_step),axis=1))
            ...
            return outputs, klds, aux_cs, zs, log_pz, log_qz
    ...
    def call(self,x, y, x_mask, return_stats=False):
        nbatch, nsteps = tf.shape(x)[0], tf.shape(x)[1]
        bwd_states, bwd_outputs = self.bwd_pass(x, y)
        fwd_outputs, klds, aux_nll, zs, log_pz, log_qz = self.fwd_pass(
            x, bwd_states=bwd_states)

        print(fwd_outputs.shape)
