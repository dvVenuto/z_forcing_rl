...
class Z_Forcing_RIMs(object):
    def __init__(self, input_dim, embedding_dim, rnn_dim, mlp_dim, z_dim, out_dim, output_type="gaussian",cond_ln=False, num_layers=1,z_force=True,embedding_dropout=0. ):
        ...
        cell_bwd = RIMCell(units=self.rnn_dim, nRIM=6, k=4, num_input_heads=1, input_key_size=32, input_value_size=32, input_query_size=32, input_keep_prob=0.9, num_comm_heads=4, comm_key_size=32, comm_value_size=32, comm_query_size=32, comm_keep_prob=0.9)

        self.bwd_mod = tf.keras.layers.RNN(cell_bwd,return_sequences=True)
        if not cond_ln:
            self.fwd_mod = tf.keras.layers.LSTM(self.rnn_dim, return_state=True)
        else:
        ...
        self._aux_mod = tf.keras.Sequential([
            tf.keras.Input(shape=(self.z_dim + self.rnn_dim)),
            tf.keras.layers.Dense(self.mlp_dim,activation='relu'),
            tf.keras.layers.Dense(units=12 * self.rnn_dim),
        ])
