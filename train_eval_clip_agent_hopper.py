...
flags.DEFINE_string('env_name', 'Hopper-v2', 'Name of an environment')
...
def train_eval(
  ...
  with tf.compat.v2.summary.record_if(
    ...
    if use_rnns:
      ...
      print("TF ENV OBS SPEC")
      input_spec = tf_env.observation_spec()

      print(input_spec)
      #print(tf.TensorShape([input_spec.shape[0]*2]))
      #print(input_spec.shape)
      #input_spec.shape = tf.TensorShape([input_spec.shape[0]*2])

      actor_net_f = actor_distribution_network.ActorDistributionNetwork(
          input_spec,
          tf_env.action_spec(),
          fc_layer_params=actor_fc_layers,
          activation_fn=tf.keras.activations.tanh)
      value_net_f = value_network.ValueNetwork(
          input_spec,
    ...
    tf_agent = ppo_clip_agent.PPOClipAgent(
        ...
        actor_net=actor_net,
        value_net=value_net,
        actor_net_f=actor_net_f,
        value_net_f=value_net_f,
    ...
    env_gen_policy = tf_agent.collect_policy
    ...
    def train_step():
      ...
      loss_info, _ = tf_agent.train(experience=trajectories)
      return loss_info
