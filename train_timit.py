...
def evaluate(dataset, model, split='valid'):
    ...
    for x, y, x_mask in get_batch():
        ...
        y = Variable(torch.from_numpy(y), volatile=True).float()
        x_mask = Variable(torch.from_numpy(x_mask), volatile=True).float()
...
def train(expname, num_layers, seed, num_epochs, rnn_dim, data, bsz, lr, z_dim,
    ...
    model = Z_Forcing(200, emb_dim, rnn_dim, z_dim,
                     mlp_dim, 400, num_layers=num_layers,
                     cond_ln=cond_ln)
    ...
    opt = tf.keras.optimizers.Adam(learning_rate=lr, epsilon=1e-05, name='Adam')
    ...
    for epoch in range(num_epochs):
        ...
        for x, y, x_mask in timit.get_train_batch():
            ...
            with tf.GradientTape() as tape:
                ...
                # x_mask = tf.Variable(torch.from_numpy(x_mask)).float()
                x = tf.Variable(tf.convert_to_tensor(x))
                y = tf.Variable(tf.convert_to_tensor(y))
                x_mask = tf.Variable(tf.convert_to_tensor(x_mask))
                hidden = (np.ones((40, 32, 1024)),np.ones((40, 32, 1024)))
                print("WORKING")
                # compute all the states for forward and backward
                # print("X SHAPE: [40, 32, 200]")
                # print(type(x))
                # print(x.shape)
                # print("Y SHAPE: [40, 32, 200]")
                # print(y.shape)
                # print("X MASK: [40, 32]")
                # print(x_mask.shape)
                fwd_nll, bwd_nll, aux_nll, kld = model.call(x, y, x_mask, hidden)
            ...
            print("ALL LOSS")
            print(all_loss)
            grads = tape.gradient(all_loss, model.trainable_weights)  
