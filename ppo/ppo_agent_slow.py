...
class PPOAgent(tf_agent.TFAgent):
  ...
  def __init__(
    ...
    self.bwd_mod = tf.keras.Sequential([
        tf.keras.layers.LSTM(20, return_sequences=True, input_shape=(None,10))
    ])

    #self.bwd_mod = tf.keras.layers.LSTM(20, return_sequences=True)

    self.bwd_out_mod = tf.keras.layers.Dense(units=17)
  ...
  def _train(self, experience, weights):
    ...
    print(tf.shape(processed_experience.observation))

    print("BACKWARDS PASS")
    bwd_states, bwd_outputs = self.bwd_pass(processed_experience.observation)
    print(tf.shape(bwd_outputs))
