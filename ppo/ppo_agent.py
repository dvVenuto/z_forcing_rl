...
class PPOAgent(tf_agent.TFAgent):
  ...
  def __init__(
      ...
      actor_net_f: Optional[network.Network] = None,
      value_net_f: Optional[network.Network] = None,
    ...
    self._actor_net_f = actor_net_f
    self._value_net_f = value_net_f
    ...
    collect_policy_foward = ppo_policy.PPOPolicy(
        time_step_spec=time_step_spec,
        action_spec=action_spec,
        actor_network=actor_net_f,
        value_network=value_net_f,
        observation_normalizer=self._observation_normalizer,
        clip=False,
        collect=True,
        compute_value_and_advantage_in_train=(
    ...
    self.bwd_mod = tf.keras.Sequential([
        tf.keras.layers.LSTM(self.rnn_dim, return_sequences=True)
    ])

    #self.bwd_mod = tf.keras.layers.LSTM(20, return_sequences=True)

    self.bwd_out_mod = tf.keras.layers.Dense(units=self.env_units)
  ...
  def gaussian_kld(self, mu_left, logvar_left, mu_right, logvar_right):
      ...
      """
      mu_left = tf.cast(mu_left, tf.float32)
      logvar_left = tf.cast(logvar_left, tf.float32)
      mu_right = tf.cast(mu_right, tf.float32)
      logvar_right = tf.cast(logvar_right, tf.float32)
  ...
  def get_loss(self,
               ...
               training: bool = False,
               bwd_states = None) -> tf_agent.LossInfo:
    ...
    distribution_step = self._collect_policy._distribution(  # pylint: disable=protected-access
        time_steps,
        policy_state,bwd_states=bwd_states,
        training=training)


    pri_mu_frwd = self._collect_policy.get_pri_mu()
    pri_lv_frwd = self._collect_policy.get_log_var()


    pri_mu_collector = self._collect_policy.get_pri_mu_col()
    pri_lv_collector = self._collect_policy.get_log_var_col()
  ...
  def bwd_pass(self, x):
      ...
      #x_bwd = tf.gather(
      #    tf.convert_to_tensor(x), idx, axis=1
      #)
  ...
  def _train(self, experience, weights):
    ...
    #print("EXPERIENCE PROC")
    #print(processed_experience.observation)

    #print("BACKWARDS PASS")
    bwd_states, bwd_outputs = self.bwd_pass(processed_experience.observation)
    #print(bwd_outputs)
    ...
    #print("TRAIN WEIGHTS")
    #print(self._actor_net.trainable_weights)
    #print(self._collect_policy._pri_mod.trainable_weights)
    # ValueError: Weights for model sequential_2 have not yet been created. 
    # Weights are created when the Model is first called on inputs or `build()` is called with an `input_shape`.
    #print(self._emb_mod.trainable_weights)
    #print(self.bwd_out_mod.trainable_weights)
    ...
    variables_to_train = list(
        object_identity.ObjectIdentitySet(self._actor_net.trainable_weights +
                                          ...
                                          self._collect_policy._pri_mod.trainable_weights+
                                          self._collect_policy._gen_mod.trainable_weights+
                                          self._collect_policy._pri_mod_col.trainable_weights+
