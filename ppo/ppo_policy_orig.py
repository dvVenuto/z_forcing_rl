...
class PPOPolicy(actor_policy.ActorPolicy):
    ...
    def __init__(self,
        ...
        self.gtime_step_spec = time_step_spec
    ...
    def get_pri_mu_col(self):
        return self.pri_mu_col
    ...
    def _apply_actor_network(self, time_step, policy_state, training=False, bwd_states=None):
        observation = time_step.observation

        # tf.print(tf.shape(observation))
        if self._observation_normalizer:
            observation = self._observation_normalizer.normalize(observation)

        outer_rank = nest_utils.get_outer_rank(
            observation, self._input_tensor_spec)
        batch_squash = utils.BatchSquash(outer_rank)

        observation = tf.nest.map_structure(batch_squash.flatten, observation)

        if bwd_states is None:
            print("NO BACKWARDS STATES-USE COL")

            pri_params = self._pri_mod(observation)

            pri_params = tf.clip_by_value(pri_params, -8., 8.)
            pri_mu, pri_logvar = tf.split(pri_params, 2, axis=1)

            eps = tf.random.normal([1])
            z_step = self.reparametrize(pri_mu, pri_logvar, eps=eps[0])
        ...
        # observation_in = observation

        if bwd_states is not None:
            print("FOUND BACKWARDS STATES")

            outer_rank = nest_utils.get_outer_rank(
                bwd_states, self._input_tensor_spec)
            batch_squash = utils.BatchSquash(outer_rank)

            bwd_states = tf.nest.map_structure(batch_squash.flatten, bwd_states)

            pri_params = self._pri_mod(bwd_states)
    ...
    def _distribution(self, time_step, policy_state, bwd_states=None, training=False):
